...
import static de.nec.nle.siafu.office.Constants.Fields.*;
...
public class AgentModel extends BaseAgentModel {
	...
	public ArrayList<Agent> createAgents() {
		...
		/** **/
		createSensors(people, "Sensor");
		createActuators(people, "Actuator");
		return people;
	}



	private void createSensors(ArrayList<Agent> people, String type) {
	}

	private void createActuators(ArrayList<Agent> people, String type) {
	}
	...
	private void createWorker(final ArrayList<Agent> people,
		...
		while (deskIt.hasNext()) {
			...
			if (type.equals("Staff")) a.set(HAS_MHUB, new BooleanType(true)); else a.set(HAS_MHUB, new BooleanType(false));
			people.add(a);
			i++;
		}
	}
	...
	public void doIteration(final Collection<Agent> agents) {
		...
		while (peopleIt.hasNext()) {
			...
			if (((BooleanType) a.get(HAS_MHUB)).getValue())
				handleMHhub(a,agentsCopy);
			handlePerson(a, now);
		}
	}

	private void handleMHhub(Agent a, Collection<Agent> agentsCopy) {
